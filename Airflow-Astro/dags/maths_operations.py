...
def substract_three(**context):
    ...
    context["ti"].xcom_push(key="current_value", value=new_value)
    print(f"Substract 3: {current_value} - 3 = {new_value}")

def square_number(**context):
    current_value = context["ti"].xcom_pull(key="current_value", task_ids="substract_three_task")
    new_value = current_value ** 2
...
with DAG(
    ...
    substract_three_task = PythonOperator(
        task_id = "substract_three_task", 
        python_callable = substract_three, 
        provide_context = True
    )
    ...
    # Dependencies. 
    start_task >> add_five_task >> multiply_by_two_task >> substract_three_task >> square_number_task